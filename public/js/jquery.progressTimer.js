(function ($) {
    $.fn.progressTimer = function (options) {
        ...
        this.each(function () {
            $(this).empty();
            var barContainer = $("<div>").addClass("progress active progress-striped xs");
            var bar = $("<div>").addClass("progress-bar progress-bar-success").addClass(settings.baseStyle)
                .attr("role", "progressbar")
                .attr("aria-valuenow", settings.startTime)
            ...
            var interval = window.setInterval(function () {
                var elapsed = new Date() - start + settings.startTime * 1000;
